function [mobility,costs, Scheme_number_count] = simulate_micro_macro(mobility, car_state, numTimesteps, N_xy_micro, N_xy_macro,paging_rate, TAL_scheme_num, history_length)
%
%
%
%
...
for i = 1:length(car_state)
    for j = 1: length(car_state{i}) 
        ...
        if ~ isstruct(mobility_last) || i==1
           ...
           if mobility_current.is_Macro   
               mobility.(['Car_',int2str(car_state{i}(j))]).(['time_',int2str(i)]).TAL = mobility_current.TAI_macro;
               mobility.(['Car_',int2str(car_state{i}(j))]).(['time_',int2str(i)]).TAI_history(1,1) = mobility_current.TAI_macro;
           else
               mobility.(['Car_',int2str(car_state{i}(j))]).(['time_',int2str(i)]).TAL = mobility_current.TAI_micro;
               mobility.(['Car_',int2str(car_state{i}(j))]).(['time_',int2str(i)]).TAI_history(1,1) = mobility_current.TAI_micro;

           end            
            
            % for new comers, we need to initialize several "history information"
            %for example: TA residential time & a new parameter--directions            
           ...
           %%                                            Part - Active UE
               ...
               % in active state, only handover would cause TAU
                if ~ ismember(mobility_current.TAI, mobility_last.TAL)
                    ...
                    [Type, N, direction_history] = TAL_optimisation_imp_micro_macro(mobility_temp, paging_rate, N_xy_micro, N_xy_macro, TAL_scheme,history_length);
                    ...
                    %%
                    TAL = TAL_scheme_assign(mobility, car_state, i, j,  N_xy_micro, N_xy_macro, TAL_scheme, ...
                ...
                if mobility_last.TAL(1)>1000
                    mobility_last_TAI = mobility_last.TAI_macro;
                    if ismember(mobility_current.TAI_macro, mobility_last.TAL)
                        mobility_current_TAI = mobility_current.TAI_macro;
                        if ~mobility_current_TAI == mobility_last_TAI
                            action = 'update history information';
                        else
                            action = 'purely update state information';
                        end
                    else
                        action = 'TAU';
                        if mobility_current.is_Macro
                            ...
                            mobility_current_TAI = mobility_current.TAI_micro;
                        end
                    end
                else
                    mobility_last_TAI = mobility_last.TAI_micro;
                    if ismember(mobility_current.TAI_micro, mobility_last.TAL)
                        mobility_current_TAI = mobility_current.TAI_micro;
                        if mobility_current_TAI ~= mobility_last_TAI
                            action = 'update history information';
                        else
                ...
                switch action
                    ...
                    mobility.(['Car_',int2str(car_state{i}(j))]).(['time_',int2str(i)])...
                        .directions = mobility_last.directions;
                    mobility.(['Car_',int2str(car_state{i}(j))]).(['time_',int2str(i)])...
                        .directions(end+1) = mobility_last_TAI-mobility_current_TAI;
                    
                    mobility.(['Car_',int2str(car_state{i}(j))]).(['time_',int2str(i)])...
                    .TAI_history = mobility_last.TAI_history;
                    mobility.(['Car_',int2str(car_state{i}(j))]).(['time_',int2str(i)])...
                    .TAI_history(end+1,1) = mobility_current_TAI;  
                    mobility.(['Car_',int2str(car_state{i}(j))]).(['time_',int2str(i)])...
                    .TAI_history(end,2) = 1;
                    ...
                    [Type, N, TAL_size,direction_history] = TAL_optimisation_imp_micro_macro...
                        (mobility_temp, N_xy_micro, N_xy_macro, paging_rate, TAL_scheme, history_length);
                    
                    % then we use the optimized parameter to distributed proper TAs to it. 
                    TAL = TAL_scheme_assign_imp_micro_macro(mobility_current_TAI,  ...
                        N_xy_micro, N_xy_macro, TAL_scheme, Type, N, direction_history,TAL_size);
                    
                    if mobility_current_TAI>1000
                        TAL = TAL+1000;
                    end
                    mobility.(['Car_',int2str(car_state{i}(j))]).(['time_',int2str(i)]).TAL= TAL;
                    %%%%%%%%%%%%%%%%%%%%  The end   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    %% for better analysing, we record information about TAU
                    if strcmp(Type, 'big cone')
                        Type = 'big_cone';
                        Scheme_number_count.(Type)(end+1) = length(TAL_size);
                    else
                        Scheme_number_count.(Type)(end+1) = length(TAL_size);
                    end
                    ...
                    case 'purely update state information'
                    %% parameter-direction is not changed, while
                    %parameter-TAIs with its timer would be changed
                    mobility.(['Car_',int2str(car_state{i}(j))]).(['time_',int2str(i)])...
                         .directions = mobility_last.directions;
                    ...
                    case 'update history information'
                    %% TAI changed brings a new direction to UE's history
                    % information
                    mobility.(['Car_',int2str(car_state{i}(j))]).(['time_',int2str(i)])...
                        .directions = mobility_last.directions;
