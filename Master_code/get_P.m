function [P, P_equibilium, Cost] = get_P(lambda, my, sigma_tau, sigma_paging, n, type, p_linear, p_conical_left, p_conical_right)
...
if n>1
    P = getMarkovTransitionProbability(n,type, p_linear, p_conical_left, p_conical_right, p_call);
    P_equibilium = [P' - eye(size(P')); ones(1,size(P',2))] \ [zeros(size(P',1),1); 1];
else
    P = 1;
    P_equibilium = 1;
end
