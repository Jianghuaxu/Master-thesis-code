function [mobility,Car_state, newVehicles_sort] = getCarMobility_micro_macro (numTimesteps,...
   numVehiclesPerTimestep, vehiclesArray, TA_num, TA_num_macro, TA_radius, TA_radius_macro, Areas)
    ...
    carID_reserved = carID_num(1001:2000);
    for i = 1:length(newVehicles(:,2))
       if ~ismember(newVehicles(i,2),carID_reserved)
           newVehicles(i,2) = 0;
       end
    end
    newVehicles(find(newVehicles(:,2)==0),:) = [];
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% assign each geo location with a specific TA ID(micro_TAI)
    ...
    %% %%%%%%%%%%%%%%%%%%%%%%%%%NEW!!!!%%%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % then Macro_TAI would be assigned
    TA_radius_y_macro = 2*TA_radius_macro/sqrt(3);
    for i = 1:length(newVehicles)
        for j = 1: length(TA_num_macro)
            delta_x = abs(newVehicles(i, 3)-TA_num_macro(j,2));
            delta_y = abs(newVehicles(i, 4)-TA_num_macro(j,3));
            if delta_x >TA_radius_macro || delta_y > TA_radius_y_macro
                continue;
            elseif (TA_radius_y_macro - delta_y)*sqrt(3)<=delta_x
                continue;
            else
                newVehicles(i, 6) = TA_num_macro(j, 1);
            end
        end
    end
    ...
    newVehicles(find(newVehicles(:,6)==0),:) = []; 
    ...
    for i = 1: length(carID_num)
        ...
        for j = 1: length(idx)
            ...
            mobility.(carID_char{i}).(timeID_char{j}).TAI_micro = newVehicles_sort(idx(j), 5);
            mobility.(carID_char{i}).(timeID_char{j}).TAI_macro = newVehicles_sort(idx(j), 6);
    ...
    %         newVehicles_sort(idx(j), 8);
            ...
            if ismember(mobility.(carID_char{i}).(timeID_char{j}).TAI_macro, Areas)
                mobility.(carID_char{i}).(timeID_char{j}).is_Macro = 1;
            else
                mobility.(carID_char{i}).(timeID_char{j}).is_Macro = 0;
            end
        end
    end
...
% end
    ...
    temp = 1;
    Car_state = cell(length(numVehiclesPerTimestep_new),1);
    idx=[];
    for i = 1: length(numVehiclesPerTimestep_new)
