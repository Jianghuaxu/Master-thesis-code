...
function [type, N, direction_temp] = TAL_optimisation_simp(mobility_current, paging_rate, TAL_scheme,history_length)
    ...
    switch TAL_scheme
        case 'Mixed'
            ...
            if length(temp_directions)<history_length 
                ...
                for i = 1:length(temp_directions)
                    temp_directions(i,2) = 0.5*(temp(end-history_length+i-1, 2)+...
                        temp(end-history_length+i, 2));
                end
            end

            six_directions(:,1)=[1, -10, -11, -1, 10,11];
            for i = 1: 6
                six_directions(i,2) = sum(temp_directions(find(temp_directions(:,1)==six_directions(i,1)),2));
            end
...
%             
            ...
            direction_temp = six_directions(p_index,1);
...
%                     cost = zeros(n);
%                     for i = 1:n
%                       cost(i)  = get_cost_simp(lambda, my, sigma_tau, sigma_paging, i, type, six_directions, p_index);
%                     end
%                     %plot(1:n, cost);
