function TAL = TAL_scheme_assign_imp_micro_macro(current_TAI,  N_xy_micro, N_xy_macro, TAL_scheme, Type, N, direction_history,TAL_size)

    if current_TAI<1000
        Nx = N_xy_micro(1); Nx2 = N_xy_micro(2);
        Ny = N_xy_micro(3); Ny2 = N_xy_micro(4);
        direction_history(:,2) = [];
    else
        current_TAI = current_TAI-1000;
        Nx = N_xy_macro(1); Nx2 = N_xy_macro(2);
        Ny = N_xy_macro(3); Ny2 = N_xy_macro(4);
        direction_history(:,1) = [];
    end
        ...
        switch Type
            ...
            TAL = cell_judge_imp(cell_in_direction_2, ref_direction(2)*i+current_TAI_pos(1), TAL, new_cells_up, Cell_num_cal, TAI_for_judge,TAL_size);
               ...
               if temp ==1
                   ...
                   if i >=2
                        for m = 2:i
                            temp_1 = current_TAI - direction_history(1,1)*m;
                            temp_2 =  current_TAI-direction_history(2,1)*m;
                            for k = 1: m-1
                                if temp_1 < temp_2 
                                    temp = temp_1+k;
                   ...
                   if i >=2
                        m = i:-1:2;
                        for k = 1:i-1    
                             TAI_for_judge = TAI_for_judge_ref+TAI_for_judge_ref2*k;
                             temp_5 = current_TAI- temp_3*k;
                             temp_6 = temp_5-temp_4*(m(k)-1);
                             TAL = cell_judge_imp(temp_6, ref_direction(3)*k+current_TAI_pos(1), TAL, new_cells_up, Cell_num_cal,TAI_for_judge, TAL_size);
                        end
                   end                               
               end
            end
            ...
            for i = 1:(N+1)
                for k = 1:(N+1)
                    if i <=k
                        TAI_for_judge_ref(i,k) = k*k-i+1;
                    else
                        TAI_for_judge_ref(i,k) = (i-1)*(i-1)+k;
                     end
                end
            end
            % firstly we need to decide the reference position(current_TAI); 
            ...
            for i = 0:N
                ...
                for k = 1:N
                    cell_temp = cell_in_direction_2_base + direction_history(1,1)*k;
                    TAI_for_judge = TAI_for_judge_ref(end-k,i+1);
                    TAL = cell_judge_imp(cell_temp, -ref_direction(1)*k+current_TAI_pos(1)+ref_direction(1)*N+...
