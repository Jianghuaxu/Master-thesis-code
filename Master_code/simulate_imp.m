function [mobility,costs, Scheme_number_count] = simulate_imp(mobility, car_state, numTimesteps, Nx, Nx2, Ny, Ny2,paging_rate, TAL_scheme_num, history_length)
%
%
%
%
           ...
           %%                                            Part - Active UE
               ...
               % in active state, only handover would cause TAU
                ...
                if ~ ismember(mobility_current.TAI, mobility_last.TAL)
                    ...
                    % how large should be decided and so TAL optimisation should be conducted.                    
                    mobility_temp = mobility.(['Car_',int2str(car_state{i}(j))]).(['time_',int2str(i)]);
                    [Type, N, TAL_size,direction_history] = TAL_optimisation_imp(mobility_temp, paging_rate, TAL_scheme, history_length);     
                    
                    % then we use the optimized parameter to distributed proper TAs to it. 
                    TAL = TAL_scheme_assign_imp(mobility, car_state, i, j, Nx,Ny, Nx2, Ny2, TAL_scheme, ...
                        Type, N, direction_history,TAL_size);
                     mobility.(['Car_',int2str(car_state{i}(j))]).(['time_',int2str(i)])...
                        .TAL= TAL;
